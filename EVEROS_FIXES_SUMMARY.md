...
### 🐛 Bug #0（致命）：StarTools.get_data_dir() 在子模块中抛出 RuntimeError

**严重程度**：🔴🔴 致命  
**影响范围**：所有 LLM 工具调用的用户 ID 持久化 —— **导致整个修复链路断掉**

**日志证据**：
```
[WARN] [tools.everos_tools:44]: [EverOS] 持久化用户 ID 失败: 
  无法获取模块 data.plugins.astrbot_plugin_everos_integration_rework.tools.everos_tools 的元数据信息
```

**根因分析**（通过探索 AstrBot 源码 `StarTools.get_data_dir()` 确认）：

```python
# AstrBot star_tools.py 第 226-240 行逻辑：
frame = inspect.currentframe().f_back    # 获取调用者的栈帧
module = inspect.getmodule(frame)        # 获取调用者模块
metadata = star_map.get(module.__name__) # 在 star_map 中查找

# star_map 的 key 来自 @register 装饰器：
star_map[cls.__module__] = metadata      # key = 插件主模块名
```

- `star_map` key = `"data.plugins.astrbot_plugin_everos_integration_rework"` （主模块）
- 工具代码在 `tools/everos_tools.py` → `inspect.getmodule` → `"data.plugins....tools.everos_tools"` （子模块）
- `star_map.get(子模块名)` → **None** → `RuntimeError("无法获取模块...的元数据信息")`
- 异常被 `_track_user_to_file` 的 `except` 捕获 → **用户 ID 从未写入文件**
- Dashboard 的 `_get_candidate_uids` 读不到 → **记录永远不可见**

**修复方案**：
不能再从子模块调用 `StarTools.get_data_dir()`。改为由插件主类在 `__init__` 中（此时还在主模块上下文，调用正常）显式注入 `data_dir`：

```python
# tools/everos_tools.py — 接收注入
...
def _track_user_to_file(user_id: str) -> None:
    if not _data_dir:  # 用注入的路径替代 StarTools.get_data_dir()
        return
    path = Path(_data_dir) / _KNOWN_USERS_FILE
    ...

# main.py — 在 __init__ 中注入
self.data_dir = str(StarTools.get_data_dir())  # ← 这里还能正常工作
everos_tools_module.set_data_dir(self.data_dir) # ← 注入到工具模块
```
...
| 文件 | 变更 |
|------|------|
| `tools/everos_tools.py` | 移除 `StarTools` 依赖；新增 `set_data_dir()` 注入 + `_track_user_to_file()` 使用注入路径 |
| `main.py` | `__init__` 中调用 `everos_tools_module.set_data_dir()` 注入；`_get_candidate_uids` 改为每次从文件读取；`api_status`/`api_memories`/`api_memories_by_type` 改用 `memory_get_all()`；新增 `_load_known_users`/`_save_known_users`；修复 `_normalize_item` |
| `core/standalone_server.py` | 新增 `_fetch_all_memories_for()`；`_get_candidate_uids` 改为每次从文件读取；所有 API 改用全量翻页 |
| `core/everos_client.py` | 新增 `memory_get()` 的 `page`/`page_size` 参数；新增 `memory_get_all()` |
| `core/config_manager.py` | 新增 `extra_user_ids`、`boundary_token_limit`、`boundary_msg_limit` 配置项 |
| `main.py` | 同步修复 `/everos flush` 输出使用配置值 |
...
### 🐛 Bug #6（已修复）：硬编码的边界检测阈值 "50 条 / 8192 token"

**严重程度**：🟡 中
**影响范围**：`/everos flush` 命令输出 + 用户对自动提炼触发条件的认知

**根因**：
`main.py` `cmd_everos_flush` 中硬编码了 `"50 条/8192 token"`。
实际 EverOS 默认值是 `65536 token / 500 msg`（定义在 `default.toml [boundary_detection]` 中）。
二者相差巨大（8192 vs 65536 token，50 vs 500 条），会误导用户。

**修复方案**：
1. 插件配置新增 `boundary_token_limit`（默认 65536）和 `boundary_msg_limit`（默认 500）
2. `/everos flush` 输出使用 `self.config.boundary_*` 动态读取
3. 配置项与 EverOS `default.toml [boundary_detection]` 默认值保持同步
...
POST /api/v1/memory/add
  → memorize(is_final=False)
  → prepare_cells: 合并缓冲 + 新鲜消息
  → _detect: 调用 everalgo BoundaryDetector
  → 超过 hard_msg_limit 或 hard_token_limit？→ 提取 cells → status="extracted"
  → 未超过？→ 消息留在缓冲区 → status="accumulated"
```

**结论：EverOS 的自动提炼功能正常工作。** 当 `memory_add` 积累的消息超出边界阈值时，会自动触发提取。

**插件当前行为：**
LLM 工具每次调用 `memory_add` 后立即调用 `memory_flush`（`is_final=True`），强制立即提取。
这种设计对于"单次记住一件事"的场景是合理的——用户不希望记忆躺在缓冲区里等几天。

**如需利用自动积累（不立即 flush）：**
将工具中的 `memory_flush` 调用删除/条件化，让 EverOS 自然积累到阈值后自动提取。
但这意味着在达到阈值前记忆不会被提取，适合批量日志场景而非单次记忆存储。
